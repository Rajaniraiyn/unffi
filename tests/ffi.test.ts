// ---- test runner: bun:test (preferred) or node:test (fallback) ----
let describe: any, test: any, expect: any, beforeAll: any, afterAll: any
try {
  ({ describe, test, expect, beforeAll, afterAll } = await import('bun:test'))
} catch {
  const nt = await import('node:test')
  const as = await import('assert')
  describe = nt.describe
  test = nt.test
  beforeAll = nt.before
  afterAll = nt.after
  expect = (v: unknown) => ({
    toBe: (e: unknown) => as.strictEqual(v, e),
    toBeUndefined: () => as.strictEqual(v, undefined),
    not: { toThrow: (fn: () => void) => as.doesNotThrow(fn) },
    toEqual: (e: unknown) => as.deepStrictEqual(v, e),
  })
}
...
async function loadBunAdapter() {
  return import('../src/adapters/bun.js')
}

// ---- main test suite (Bun runtime only, uses bun:ffi) ----
if (IS_BUN) {
  const { dlopen, t } = await loadBunAdapter()
...
} // end IS_BUN main suite

// ---- runtime-specific ----
if (IS_BUN) {
  const { t: tBun, dlopen } = await import('../src/adapters/bun.js')
...
if (IS_DENO) {
  const { t: tDeno, dlopen: denoDlopen } = await import('../src/adapters/deno.js')
...
if (IS_NODE) {
  ...
  const { t: tNode, dlopen: nodeDlopen } = await import('../src/adapters/node.js')
  ...
  describe('Node adapter: t.node types (native node:ffi or koffi fallback)', () => {
    test('tNode.i32 is valid', () => {
      expect(tNode.i32.kind).toBe('i32')
    })
    test('tNode.cstring is valid', () => {
      expect(tNode.cstring.kind).toBe('cstring')
    })
  })

  describe('Node adapter: core FFI operations', () => {
    test('add_i32 via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        add_i32: { args: [tNode.i32, tNode.i32], returns: tNode.i32 },
      })
      expect(l.symbols.add_i32(10, 20)).toBe(30)
    })
    test('greet via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        greet: { args: [tNode.cstring], returns: tNode.cstring },
      })
      expect(l.symbols.greet('World')).toBe('Hello, World')
    })
    test('bool via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        gt_i32: { args: [tNode.i32, tNode.i32], returns: tNode.bool },
      })
      expect(l.symbols.gt_i32(5, 3)).toBe(true)
      expect(l.symbols.gt_i32(3, 5)).toBe(false)
    })
    test('i64 via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        add_i64: { args: [tNode.i64, tNode.i64], returns: tNode.i64 },
      })
      expect(l.symbols.add_i64(1n, 2n)).toBe(3n)
    })
    test('f64 via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        add_f64: { args: [tNode.f64, tNode.f64], returns: tNode.f64 },
      })
      expect(l.symbols.add_f64(1.5, 2.5)).toBe(4.0)
    })
    test('noop via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        noop: { args: [], returns: tNode.void },
      })
      expect(l.symbols.noop()).toBeUndefined()
    })
    test('callbacks via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        apply: { args: [tNode.fn([tNode.i32], tNode.i32), tNode.i32], returns: tNode.i32 },
      })
      expect(l.symbols.apply((x: number) => x * 2, 5)).toBe(10)
    })
    test('cstring callback via node adapter', async () => {
      await using l = nodeDlopen(LIB, {
        with_message: { args: [tNode.fn([tNode.cstring], tNode.void), tNode.cstring], returns: tNode.void },
      })
      let received: string | null = null
      l.symbols.with_message((msg: string) => { received = msg }, 'hello')
      expect(received).toBe('hello')
    })
  })
}
